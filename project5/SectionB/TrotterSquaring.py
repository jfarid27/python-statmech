...
def analytical_density(x, beta):
    """
    Analytical density function of quantum simulator
    """
    return math.sqrt(math.tanh(float(beta)/2) / math.pi) \
       * math.exp( -( x ** 2) * math.tanh(float(beta)/2) )
...
f = open('data_harm_matrixsquaring_beta' + str(beta_tmp) + '.dat', 'w')
for j in range(nx + 1):
    f.write(str(x[j]) + ' ' + str(rho[j, j] / Z) + '\n')
f.close()

z = [analytical_density(a, beta) for a in x]
pylab.plot(x, pi_of_x, c='red', linewidth=2.0)
pylab.plot(x, z, c='blue', linewidth=.50)
pylab.title('Theoretical and computed Trotter distribution $\pi(x)$ \
    \nhistogram for beta='+str(beta), fontsize = 18)
